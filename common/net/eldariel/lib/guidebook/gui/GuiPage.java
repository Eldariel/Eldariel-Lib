...
import net.eldariel.lib.guidebook.page.ItemDisplay;
...
public class GuiPage extends GuiGuide {
			...
			if (Desktop.isDesktopSupported()) {
				for (String link : links) {
					try {
						...
						EldarielLib.logHelper.warning("Couldn't open website from guide book page!");
						e.printStackTrace();
					}
				}
			}
	...
	public void initGui() {
		...
		if (links != null && !links.isEmpty()) {
			this.buttonViewOnline = new TexturedButton(book.getResourceGadgets(), -782822, this.guiLeft + this.xSize - 24, this.guiTop + this.ySize - 25, 0, 172, 16, 16, Collections.singletonList(TextFormatting.GOLD + book.loc.getLocalizedString("guide", "onlineButton.name")));
		...
		if (page != null) {
			...
			if (chapter != null) {
				...
				if (pageNumToOpen >= 0 && pageNumToOpen < pages.length) {
					this.mc.displayGuiScreen(GuideBookUtils.createPageGui(book, this.previousScreen, this.parentPage, pages[pageNumToOpen]));
				}
			}
		}
	}
