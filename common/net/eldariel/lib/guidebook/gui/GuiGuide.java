...
public abstract class GuiGuide extends GuiGuideBase {
	...
	public void initGui() {
		...
		if (this.hasBackButton()) {
			...
			this.buttonBack = new TexturedButton(book.getResourceGadgets(), -2002, this.guiLeft - 15, this.guiTop - 3, 36, 54, 18, 10, hoverText);
		...
		// PlayerSave data = PlayerDate.getDataFromPlayer(this.mc.player);
		//
	...
	public void drawScreenPre(int mouseX, int mouseY, float partialTicks) {
		...
		if (this.hasSearchBar()) {
			...
			if (!this.searchField.isFocused() && (this.searchField.getText() == null || this.searchField.getText().isEmpty())) {
				String line = book.loc.getLocalizedString("guide.eldariellib:searchField");
				this.fontRenderer.drawString(TextFormatting.ITALIC + line, this.guiLeft + this.xSize + 2, this.guiTop + this.ySize - 40 + 2, 0xFFFFFF, false);
			}
	...
	public void onSearchBarChanged(String searchBarText) {
		...
		this.mc.displayGuiScreen(new GuiEntry(book, this.previousScreen, parent, book.entryAllAndSearch, 0, searchBarText, true));
	}
