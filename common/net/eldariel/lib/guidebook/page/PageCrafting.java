...
public class PageCrafting extends GuidePage {
	...
	public void drawScreenPre(GuiGuideBase gui, int startX, int startY, int mouseX, int mouseY, float partialTicks) {
		...
		gui.renderScaledAsciiString("(" + book.loc.getLocalizedString(this.recipeTypeLocKey) + ")", startX + 6, startY + 65, 0, false, gui.getMediumFontSize());
	...
	private void setupRecipe(GuiGuideBase gui, IRecipe recipe, int startX, int startY) {
		...
		if (recipe instanceof ShapedRecipes) {
			...
			this.recipeTypeLocKey = "guide.eldariellib:shapedRecipe";
			...
			this.recipeTypeLocKey = "guide.eldariellib:shapelessRecipe";
			...
			try {
				...
				EldarielLib.logHelper.warning("Something went wrong trying to get the Crafting Recipe in the booklet to display!");
				e.printStackTrace();
			}
			...
			this.recipeTypeLocKey = "guide.eldariellib:shapedOreRecipe";
			...
			this.recipeTypeLocKey = "guide.eldariellib:shapelessOreRecipe";
		}
